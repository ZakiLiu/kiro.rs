...
impl KiroCredentials {
    ...
    pub fn fill_default_profile_arn(&mut self) -> bool {
        ...
        // Social 凭据：占位符或缺失 → SOCIAL_PROFILE_ARN
        // IdC 凭据：占位符保留不动，缺失 → BUILDER_ID_PROFILE_ARN
        ...
        } else if self.profile_arn.is_none() {
    ...
    ///
    /// Social：占位符/缺失回退 SOCIAL_PROFILE_ARN
    /// IdC：占位符不是真实 profile，返回 None
    ...
    ///
    /// Social：占位符/缺失回退 SOCIAL_PROFILE_ARN
    /// IdC：返回真实 ARN 或 BUILDER_ID 占位符（上游需要字段存在）
...
mod tests {
    ...
    #[test]
    fn test_effective_profile_arn_idc_placeholder_returns_none() {
