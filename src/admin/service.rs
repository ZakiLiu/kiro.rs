...
impl AdminService {
    ...
    pub async fn start_external_idp_login(
        &self,
        req: super::types::StartExternalIdpLoginRequest,
    ) -> Result<serde_json::Value, AdminServiceError> {
        ...
        let oidc_config =
            external_idp::oidc_discovery(&req.issuer_url, proxy.as_ref(), tls_backend)
                .await
                .map_err(|e| AdminServiceError::InternalError(e.to_string()))?;

        // PKCE + callback server
        let (code_verifier, code_challenge) = social::generate_pkce();
        let state = uuid::Uuid::new_v4().to_string();
        ...
        let scopes = req
            .scopes
            .unwrap_or_else(|| external_idp::default_sso_scopes(&req.client_id));

        let auth_url = external_idp::build_authorization_url(
        ...
        let cred_template = KiroCredentials {
            ...
            token_endpoint: Some(oidc_config.token_endpoint.clone()),
            issuer_url: Some(req.issuer_url.clone()),
            scopes: Some(scopes),
        ...
        let session = SocialAuthSession {
            ...
            auth_endpoint: oidc_config.token_endpoint.clone(),
    ...
    pub async fn poll_external_idp_login(
        ...
        // 复用 social poll 逻辑（共享 social_sessions 存储）
        self.poll_social_login(session_id).await
    }
}
