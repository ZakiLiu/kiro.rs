...
### Fixed
- **计算探测取最后一个未答问题** — 多轮 Q&A 格式（含示例答案）时解析器错误取了第一个已答算式，现在正确提取最后一个 `Q:` 行计算 (`src/anthropic/health_check.rs`)
...
### Fixed
- **缓存命中率计算修正** — 流式路径 `input_tokens` 从总估算改为 `billed_input_tokens`（与非流式一致），修复命中率永远卡在 ~48.5% 的问题

## [v2.0.27] - 2026-06-27

### Changed
- **模拟缓存最小 token 阈值降低** — opus 从 4096 降至 200，sonnet/haiku 从 1024 降至 100，短对话也能触发本地缓存模拟
...
### Docs
- **CHANGELOG 补全** — 补充 v2.0.16 ~ v2.0.24 的变更记录，CI Release 页面自动展示变更内容
